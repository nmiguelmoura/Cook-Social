...
class ForgotPasswordHandler(handler.Handler):
    """Class that allows the recovery of user password. It sends an email with a randomly generated password that is stored in db. This class has to change to allow the input of username instead of email address. Username is unique, email address might not be."""
    ...
    def post(self):
        ...
        if email_validation["response"]:
            ...
            if user_email:
                ...
                # Store new password in user entity associated to the email address.
