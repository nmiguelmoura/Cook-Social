...
class RecipeHandler(handler.Handler):
    ...
    def render_page(self, id, recipe, author, user_id, user_pointed, comments, alert_window=False, alert_message=""):
        # Render page with all parameters
        self.render("recipe.html", id=id, title=recipe.title, author=author, author_id=recipe.user_id,
                    image=recipe.image, prep_time=recipe.prep_time, ingredients=recipe.ingredients, steps=recipe.steps, user_id=user_id, points=recipe.points, user_pointed=user_pointed, comments=comments, alert_window=alert_window, alert_message=alert_message)
    ...
    def manage_point(self, values):
        ...
        if values["user_pointed"]:
            ...
            # If user hasn't attributed 1 point to this recipe, append recipe id to list points in user.
    ...
    def get(self):
        ...
        if values["recipe"]:
            ...
            self.render_page(values["id"], values["recipe"], values["recipe_author"], values["user_id"], values["user_pointed"], values["comments"])
        else:
            # If recipe does not exist, redirect to error message page.
            self.redirect("/messagetouser?type=notfound")

    def post(self):
        # This post function allows the current user to add or remove 1 point to recipe.
        ...
        if values["user_id"]:
            ...
            if values["user_id"] == values["recipe"].user_id:
                # If logged user id matches recipe author id, render page with alert window.
                # An author can't point his/her own recipe.
                self.render_page(values["id"], values["recipe"], values["recipe_author"], values["user_id"], values["user_pointed"], values["comments"], alert_window=True, alert_message="You can't vote in your own recipes!")
            else:
                # If logged user is not recipe author, allow to add or remove point.
