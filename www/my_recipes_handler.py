...
class MyRecipesHandler(handler.Handler):
    """Class that handles the recipes page. This page is like a blog that shows everything a user has posted."""
    ...
    def get(self):
        ...
        if get_id:
            # If get method defines a user, store user id in user_id var, to allow showing this users page.
            user_id = get_id
        else:
            # If get method does not defines a user, get user_id form currently logged user.
            user_id = self.cookies.get_loginfo_cookie(self)

            if not user_id:
                # If user is not logged in (and obviously no user id was sent through get method), redirect to loginpage.
                self.redirect("login")
                return

        # Render page related to user_id (passed through get method or got from cookie)
